...
/**
 * @brief считывает значение типа double
 * @return считанное значение целочисленное
 */
 ...
 */
void printIndexesGreaterThana(const int arr[], const int n, const double a);
...
void replaceSecondWithMaxNegative(int arr[], int n) {
    ...
    for (size_t i = 0; i < n; i++) 
    {
        if (abs(arr[i]) > abs(max_el) && arr[i] < 0) 
        {
            max_el = arr[i];
        }
    }
